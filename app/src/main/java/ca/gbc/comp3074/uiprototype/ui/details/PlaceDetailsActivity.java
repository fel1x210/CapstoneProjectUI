...
public class PlaceDetailsActivity extends AppCompatActivity {
    ...
    private void loadPlaceFromDatabase(int placeId) {
        ...
        // For now, we'll show a sample place
        showSamplePlace();
    }
    ...
    private void loadPlaceFromGoogle(String googlePlaceId) {
        placesService.getPlaceDetails(googlePlaceId, new GooglePlacesService.PlaceDetailsCallback() {
            ...
            public void onError(String error) {
                runOnUiThread(() -> {
                    Toast.makeText(PlaceDetailsActivity.this, "Error loading place details: " + error, 
                                 Toast.LENGTH_SHORT).show();
                    showSamplePlace(); // Fallback to sample data
                });
            }
        });
    }
    ...
    private void displayPlaceDetails(GooglePlacesService.GooglePlaceDetails details) {
        ...
        if (details.openingHours != null) {
            ...
            placeHours.setTextColor(getColor(details.openingHours.openNow ? 
                                           R.color.quiet_space_primary : R.color.quiet_space_warning));
        }
    ...
    private void showSamplePlace() {
        // Show sample data as fallback
        placeName.setText("The Urban Reader Café");
        placeType.setText("Café");
        placeAddress.setText("123 Queen St W, Toronto, ON");
        placeRating.setText("⭐ 4.7");
        placeReviews.setText("(362 reviews)");
        placeDistance.setText("0.2 km away");
        placeHours.setText("Open now");
        placeHours.setTextColor(getColor(R.color.quiet_space_primary));
        placePhone.setText("(416) 555-0123");
        placeDescription.setText("A cozy café perfect for reading and studying with excellent coffee and quiet atmosphere.");
        quietScoreValue.setText("4.2");
        
        // Add sample tags
        addTagsForPlaceType("Café");
    ...
    private String determineQuietSpaceType(String placeName) {
        ...
        if (lowerName.contains("cafe") || lowerName.contains("coffee")) return "Café";
        if (lowerName.contains("park")) return "Park";
        if (lowerName.contains("museum")) return "Museum";
        if (lowerName.contains("gallery")) return "Gallery";
        if (lowerName.contains("spa")) return "Wellness";
        if (lowerName.contains("church") || lowerName.contains("temple")) return "Spiritual";
        return "Quiet Space";
    }
    ...
    private float calculateQuietScore(String type, float rating) {
        ...
        switch (type) {
            case "Library": baseScore = 4.8f; break;
            case "Park": baseScore = 4.2f; break;
            case "Museum": case "Gallery": baseScore = 4.0f; break;
            case "Spiritual": baseScore = 4.5f; break;
            case "Café": baseScore = 3.0f; break;
            case "Wellness": baseScore = 4.3f; break;
        }
    ...
    private String generateDescription(String type, float rating, int reviewCount) {
        ...
        switch (type) {
            case "Library": baseDescription = "Quiet study space with books and peaceful atmosphere"; break;
            case "Park": baseDescription = "Natural outdoor space perfect for relaxation and quiet activities"; break;
            case "Café": baseDescription = "Cozy spot for coffee and quiet conversation"; break;
            case "Museum": baseDescription = "Cultural space with quiet galleries and exhibitions"; break;
            case "Gallery": baseDescription = "Artistic environment perfect for contemplation"; break;
            case "Wellness": baseDescription = "Relaxing space focused on health and tranquility"; break;
            case "Spiritual": baseDescription = "Peaceful place for reflection and meditation"; break;
            default: baseDescription = "A quiet space for relaxation and peace";
        }
    ...
    private void addTagsForPlaceType(String type) {
        List<String> tags;
        switch (type) {
            case "Library": tags = Arrays.asList("Very quiet", "Study spaces", "Free WiFi", "Books"); break;
            case "Park": tags = Arrays.asList("Fresh air", "Natural sounds", "Peaceful", "Outdoor"); break;
            case "Café": tags = Arrays.asList("Coffee", "WiFi", "Cozy atmosphere", "Light chatter"); break;
            case "Museum": tags = Arrays.asList("Quiet galleries", "Cultural", "Educational", "Peaceful"); break;
            case "Gallery": tags = Arrays.asList("Art", "Contemplative", "Inspiring", "Quiet"); break;
            case "Wellness": tags = Arrays.asList("Relaxation", "Meditation", "Tranquil", "Healing"); break;
            case "Spiritual": tags = Arrays.asList("Meditation", "Prayer", "Silence", "Sacred"); break;
            default: tags = Arrays.asList("Quiet", "Peaceful", "Relaxing");
        }
    ...
    private void setupSampleButtons() {
        callButton.setOnClickListener(v -> {
            Intent callIntent = new Intent(Intent.ACTION_DIAL);
            callIntent.setData(Uri.parse("tel:(416) 555-0123"));
            startActivity(callIntent);
        });
        
        directionsButton.setOnClickListener(v -> {
            String uri = "geo:43.6532,-79.3832?q=43.6532,-79.3832(The Urban Reader Café)";
            Intent mapIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(uri));
            mapIntent.setPackage("com.google.android.apps.maps");
            if (mapIntent.resolveActivity(getPackageManager()) != null) {
                startActivity(mapIntent);
